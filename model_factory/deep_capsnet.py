...
class FC_Caps(nn.Module):
    ...
    def __init__(self, output_capsules, input_capsules, in_dimensions, out_dimensions, device, routing_iter=3):
        ...
        self.weight = nn.Parameter(torch.randn(1, self.input_capsules, self.output_capsules, self.out_dimensions, self.in_dimensions)*0.05)
        self.bias = nn.Parameter(torch.randn(1, 1, self.output_capsules, self.out_dimensions)*0.05)
    ...
    def forward(self, x):
        ...
        u_hat = torch.matmul(self.weight, x).squeeze()
        ...
        for iter_idx in range(self.routing_iter):
            ...
            if iter_idx == self.routing_iter - 1:
                s_j = (c_ij * u_hat).sum(dim=1, keepdim=True) + self.bias #multiply with the original u_hat since we want the gradient flow.
...
class DeepCapsModel(ModelMeta, nn.Module):
    '''
    DeepCaps Model.
    '''

    def __init__(self, num_class, image_size, learning_rate, decay_rate, m_plus, m_minus, lambda_, device):
        '''
        Param Init.
        '''
    ...
    def build_model(self):
        ...
        self.conv2dcaps_00 = Conv2DCaps(height=self.height, width=self.width, conv_channel_in=128, caps_num_in=1, conv_channel_out=32,
                                        caps_num_out=4, stride=2, device=self.device)
        ...
        self.conv2dcaps_01 = Conv2DCaps(height=height, width=width, conv_channel_in=32, caps_num_in=4, conv_channel_out=32,
                                        caps_num_out=4, stride=1, device=self.device)
        ...
        self.conv2dcaps_10 = Conv2DCaps(height=height, width=width, conv_channel_in=32, caps_num_in=4, conv_channel_out=32,
                                        caps_num_out=8, stride=2, device=self.device)
        ...
        self.conv2dcaps_30 = Conv2DCaps(height=height, width=width, conv_channel_in=32, caps_num_in=8, conv_channel_out=32,
                                        caps_num_out=8, stride=2, device=self.device)
        height, width = math.ceil(height/2), math.ceil(width/2)
        self.conv3dcaps_31 = Conv3DCaps(height=height, width=width, conv_channel_in=32, caps_num_in=8, conv_channel_out=32, caps_num_out=8, device=self.device)
        ...
        self.fc_caps = FC_Caps(output_capsules=self.num_class, input_capsules=640, in_dimensions=8,
                                    out_dimensions=16, routing_iter=3, device=self.device)
    ...
    def optimize_model(self, predicted, target, ori_imgs, decoded, loss_func, optim_func, decay_func, lr_decay_step=False):
        ...
        total_loss = loss_func(x=predicted, reconstructed=decoded, data=ori_imgs, labels=target, lambda_=self.lambda_, m_plus=self.m_plus,
    ...
    def calculate_loss(self, predicted, target, ori_imgs, decoded, loss_func):
        '''
        Returns the total loss of the network.
        '''
        total_loss = loss_func(x=predicted, reconstructed=decoded, data=ori_imgs, labels=target, lambda_=self.lambda_, m_plus=self.m_plus,
                                m_minus=self.m_minus)
